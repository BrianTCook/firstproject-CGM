...
def ColdensImpactParameterPlotter(input_arrays, num_bins, covfracvalue, physical_or_virial): #boxsize in Mpc, array must be square
	...
	for i in range(num_bins-1):
		...
		if len(pixs) != 0:
			...
			output_to_display[i,6] = num_above_covfracvalue[i]

		if len(pixs) == 0:

			#2 won't shop up on coldens or covfrac plots
			output_to_display[i,2] = 2.
			output_to_display[i,3] = 2. 
			output_to_display[i,4] = 2.
			output_to_display[i,5] = 2.
			output_to_display[i,6] = 2.
